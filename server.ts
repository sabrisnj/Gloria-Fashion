...
async function startServer() {
  ...
  app.post("/api/appointments", (req, res) => {
    try {
      ...
      console.log('Creating appointment for client_id:', client_id);
      ...
      if (!client) {
        console.error('Client not found for appointment creation:', client_id);
        return res.status(404).json({ error: "Cliente não encontrado. Por favor, realize o cadastro novamente." });
      }
