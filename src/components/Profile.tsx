...
export function Profile({ client, onLogout, accessibility, setAccessibility }: ProfileProps) {
  ...
  useEffect(() => {
    if (client) {
      Promise.all([
        fetch(`/api/vouchers/${client.id}`).then(res => res.json()),
        fetch('/api/appointments').then(res => res.json()).then(data => data.filter((a: any) => a.client_id === client.id))
      ]).then(([vData, aData]) => {
        setVouchers(vData);
      ...
      <section className="space-y-4">
        ...
        <div className="grid gap-3">
          ...
          <div className="card flex items-center gap-4 hover:bg-peach/10 transition-colors cursor-pointer border-peach/20">
            <div className="w-10 h-10 bg-peach/20 text-primary rounded-full flex items-center justify-center shrink-0">
              <Camera size={20} />
            </div>
            <div>
              <p className="font-bold text-sm text-ink">Poste e Ganhe (3% OFF)</p>
              <p className="text-[10px] text-gray-custom">Poste uma foto com nosso produto e marque @glorinha_presentesepiercings.</p>
            </div>
          </div>
          <div className="card flex items-center gap-4 hover:bg-peach/10 transition-colors cursor-pointer border-peach/20">
            ...
            <GuideItem title="Cupons e Promoções" icon={Gift}>
              <div className="space-y-3 text-sm text-gray-custom">
                ...
                <div className="space-y-1 text-xs">
                  ...
                  <p>📸 <span className="font-bold">Poste e Ganhe</span> — Poste foto com produto → 7% de desconto</p>
                  <p>🔄 <span className="font-bold">Novo Voucher Social</span> — Nova postagem → 5% de desconto</p>
            ...
            <GuideItem title="Check-in na Loja" icon={QrCode}>
              <div className="space-y-2 text-sm text-gray-custom">
                ...
                <ol className="list-decimal ml-4 space-y-1">
                  ...
                  <li>Toque em "Check-in QR Code" na tela inicial</li>
                  <li>Escaneie o QR Code da Glória Fashion</li>
                  <li>Informe quem te indicou (opcional)</li>
