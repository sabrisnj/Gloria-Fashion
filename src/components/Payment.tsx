...
export function Payment({ client }: PaymentProps) {
  ...
  const [checkInType, setCheckInType] = useState<'photo' | 'manual' | null>(null);
        ...
        {!checkInType ? (
          <div className="grid grid-cols-2 gap-3">
            <button 
              ...
              className="flex flex-col items-center justify-center p-6 bg-white border-2 border-gray-100 rounded-3xl gap-3 hover:border-primary transition-all group"
            >
              <div className="w-12 h-12 bg-gray-50 rounded-full flex items-center justify-center text-gray-400 group-hover:bg-primary/10 group-hover:text-primary transition-colors">
                <Camera size={24} />
              </div>
              <span className="text-[10px] font-bold uppercase text-gray-custom group-hover:text-ink">Tirar Foto</span>
            </button>
            <button 
            ...
            >
              ...
              <span className="text-[10px] font-bold uppercase text-gray-custom group-hover:text-ink">Check-in Manual</span>
            </button>
          </div>
        ) : (
          <motion.div 
          ...
          >
            <div className="flex items-center justify-between">
              <h3 className="font-bold text-ink">
                {checkInType === 'photo' ? 'Check-in por Foto' : 'Check-in Manual'}
              </h3>
              <button 
